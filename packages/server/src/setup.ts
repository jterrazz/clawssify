...
function writeEnv(env: Record<string, string>): void {
  const lines = ['# Clawssify configuration', '# Generated by `pnpm dev` setup wizard', '']
...
async function runSetup(): Promise<void> {
  ...
  if (providerChoice === 0) {
    ...
    if (authChoice === 0) {
      ...
      env.AI_MODEL = ['claude-sonnet-4-5-20250929', 'claude-opus-4-6', 'claude-haiku-4-5-20251001'][
        modelChoice
      ]
    } else {
...
// Main — only run the wizard on first use (no .env or missing API_KEY)
const env = readEnv()
if (!isConfigured(env)) {
  await runSetup()
}
