//! System and user prompts — port of `server/src/services/ai/prompts.ts`.

pub const SYSTEM_PROMPT: &str = r#"You are Clawssify, an AI librarian managing a personal knowledge base.
...
/// Build the user prompt for a single ingestion — port of `buildUserPrompt`.
pub fn build_user_prompt(
    ...
    if let Some(u) = url {
        prompt.push_str(&format!("**Source URL**: {u}\n"));
    }

    prompt.push_str(&format!(
        "\n**Content**:\n{content}\n\n\
         Please process this content according to the impact level and your workflow. \
         Start by reading your brain context, then explore existing knowledge, \
         then create/update files."
    ));
